...
nginx_local_check=$(md5sum ./nginx/webgl.samdriver.xyz \
    | awk '{ print $1 }')
nginx_remote_check=$(ssh root@"$remote_ip" "md5sum /etc/nginx/sites-available/webgl.samdriver.xyz" \
    | awk '{ print $1 }')

if [ "$nginx_local_check" != "$nginx_remote_check" ]; then
    echo "nginx config changed, so deploying."
    rsync -avz --no-perms --no-owner --no-group \
        ./nginx/webgl.samdriver.xyz root@"$remote_ip":/etc/nginx/sites-available/webgl.samdriver.xyz
